...
// These variables will be used to monitor if the user already won in the value of 2048, 4096, 8192
let is2048Exist = false;
let is4096Exist = false;
let is8192xist = false;
...
// This function removes zeroes from the row / col
...
function slide(row) {
    ...
    for (let i=0; i<row.length -1; i++) {
        if(row[i] == row[i+1]) { // checks if a tile is equal to its adjacent tile
            row[i] *=2; // merge - double the first tile to merge
                        // [4, 2] 
            row[i + 1] = 0; // [4, 0]
    ...
    return row // submit the updated row / columns
}
...
function slideLeft() {
    for(let r = 0; r < rows; r++){
        ...
        // After merging, the position and the value of the tiles might change, thus if follows that the id, number, color of the tile must be changed
        for(let c = 0; c < columns; c++){
...
function checkWin() {
    ...
    for(let r = 0; r < rows; r++) {
        for(let c = 0; c < columns; c++) {
            ...
            else if (board[r][c] == 8192 && is8192xist == false) {
                alert("Victory! You have reach 8192! you are incredibly awesome x 8192!!");
                is8192xist = true;
            }
        }
    }
...
// hasLost function will check if there is still an empty tile (meaning, there is still a possible move) and it will also check if there a same tile value adjacent (beside/ side by side / adjacent to each other, meaning there is still possible move)
function hasLost() {
    ...
    for(let r=0; r<rows; r++) {
        for(let c=0; c<columns; c++) {
            ...
            // This code will check if there is  a tile that equal to zero(meaning, empty file)
            if (board[r][c] == 0) {
                return false;
            }
            ...
            if (
                // Check current tile if it has possible mergre to its upper tile OR
                ...
                // if we found adjacent tile with the same value as the current tile, false, the user has not lost
                return false;
    ...
    // No empty tile and no possible moves left (meaning the user already lost)
    return true;
}
...
//This code will lesten when we touch the screen and assigns the x coordinates
// Input the x coordinates value to
...
document.addEventListener('touchmove', (e) => {
    ...
    e.preventDefault()
}, {passive: false}) // User passive property to make sure that the preventDefault() will work
